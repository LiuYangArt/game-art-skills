...
默认输出三份彼此对应、都落成磁盘文件、都可直接复制的结果：

1. 能直接粘贴到 Unreal `Code` 字段的 `HLSL`
2. 能直接粘贴到 Unreal 材质编辑器里的单节点 `MaterialGraphNode_Custom` 导出文本
3. 一个尽可能完整的 `Full Graph` 导出文本

这三份文件都必须直接写成 `.md` 文件，但文件内容本身必须是“纯代码/纯导出文本”，不要在文件前面再包标题、说明段落、操作步骤、代码围栏或任何冗余文字。

在条件允许时调用 [`scripts/validate_custom_node.py`](scripts/validate_custom_node.py) 做静态语法与离线编译验证。默认优先调用 [`scripts/emit_custom_node_artifacts.py`](scripts/emit_custom_node_artifacts.py) 一次性生成三份 `.md` 文件；只有当用户明确要求“直接在对话里展开代码块”时，才考虑调用 [`scripts/render_custom_node_dual_blocks.py`](scripts/render_custom_node_dual_blocks.py) 或底层导出脚本。
...
3. 再列出节点配置：
   ...
   - `Include File Paths`
   - `Desc`
4. 必须生成一个 `HLSL Markdown` 文件，并返回完整绝对路径。
5. 必须生成一个 `Material Editor Paste Export Markdown` 文件，并返回完整绝对路径。
6. 必须生成一个 `Full Graph Markdown` 文件，并返回完整绝对路径。
7. 如果做了验证，再补一段验证结果。
...
其中三个文件必须默认全部生成，不允许省略。

默认先尽量生成“包含外围节点与连线”的 `full_graph`；如果当前信息不足以补全所有外围节点，也仍然必须输出 `full_graph.md`，至少包含当前能稳定生成的节点组，不要因为缺少 layout 就跳过这个文件。

不要先输出 `.txt` 再让调用方手动转成 `.md`。脚本和工作流都必须直接落地最终 `.md` 文件。

这三份 `.md` 文件的内容要求：

- `*_custom_node_hlsl.md`：文件内只放原始 HLSL 代码体
- `*_custom_node_paste.md`：文件内只放原始 Unreal 单节点粘贴文本
- `*_full_graph.md`：文件内只放原始 Unreal 完整节点组粘贴文本
...
不要只给短链接或相对路径。
不要把路径藏在 markdown 链接标签后面，例如不要写成 `[foo.md](C:\full\path\foo.md)` 但正文里只显示 `foo.md`。
返回给用户时，必须把完整绝对路径作为原始文本直接逐条写出来，让用户肉眼就能直接看到完整路径。
...
- 必定生成的 `HLSL Markdown`
- 必定生成的 `single-node paste markdown`
- 必定生成的 `full-graph markdown`
- 供直接返回给用户的完整绝对路径列表
- 不带代码围栏、不带说明文字的纯内容文件
...
- 默认用中文说明。
- 默认优先返回三份拆分后的 `.md` 文件完整绝对路径，而不是在对话里直接展开超长代码块。
- 默认必须同时返回这三份文件的完整绝对路径：
  - `*_custom_node_hlsl.md`
  - `*_custom_node_paste.md`
  - `*_full_graph.md`
- 只有当用户明确要求“直接贴在对话里”时，才把代码块整段展开。
- 不要在回答里只写文件名；必须写完整绝对路径。
- 不要只用 markdown 文件链接标签来展示路径；正文里必须直接出现完整绝对路径字符串本身。
- 最稳妥的返回格式是逐条单独列出，例如：
  - `E:\Portable Apps\AppData\AGENTS\.agents\skills\unreal-custom-node\SKILL.md`
  - `D:\Somewhere\artifact_custom_node_hlsl.md`
